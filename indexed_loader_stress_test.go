...
type stressMutableValue struct {
	N int
}

func (v *stressMutableValue) Clone() *stressMutableValue {
	return &stressMutableValue{N: v.N}
}
...
func TestStress_IndexedLoadingCache(t *testing.T) {
	...
	newEntry := func(key int) *loadingcache.CacheEntry[int, *stressMutableValue] {
		switch key {
		case missingPK:
			return nil
		case negativePK:
			return &loadingcache.CacheEntry[int, *stressMutableValue]{
				Entry:         loadingcache.Entry[int, *stressMutableValue]{Key: key},
				NegativeCache: true,
				ExpiresAt:     time.Now().Add(time.Hour),
			}
		default:
			return &loadingcache.CacheEntry[int, *stressMutableValue]{
				Entry:     loadingcache.Entry[int, *stressMutableValue]{Key: key, Value: &stressMutableValue{N: key * 100}},
				ExpiresAt: time.Now().Add(ttl),
			}
		}
	}
	src := &source.FunctionsSource[int, *stressMutableValue]{
		GetFunc: func(_ context.Context, key int) (*loadingcache.CacheEntry[int, *stressMutableValue], error) {
			return newEntry(key), nil
		},
		GetMultiFunc: func(_ context.Context, keys []int) ([]*loadingcache.CacheEntry[int, *stressMutableValue], error) {
			entries := make([]*loadingcache.CacheEntry[int, *stressMutableValue], len(keys))
	...
	st := memstorage.NewInMemoryStorage[int, *stressMutableValue]()
	loader := singleflightloader.NewSingleFlightLoader(st, src)
	cache := loadingcache.NewIndexedLoadingCache(
		loadingcache.LoadingCache[int, *stressMutableValue]{Loader: loader, Storage: st},
		idx,
	)
	...
	verifyAndDestroy := func(sk uint8, entries []*loadingcache.Entry[int, *stressMutableValue]) error {
		found := 0
		for _, entry := range entries {
			...
			if got := entry.Value.N; got != entry.Key*100 {
		...
		eg.Go(func() error {
			for i := 0; i < numIterations; i++ {
				...
				if i%16 == 15 {
					...
					if entries, err := cache.FindBySecondaryKey(ctx, sk); err == nil {
